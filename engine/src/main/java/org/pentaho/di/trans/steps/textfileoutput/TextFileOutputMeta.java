...
public class TextFileOutputMeta extends BaseFileOutputMeta implements StepMetaInterface, ResolvableResource {
	...
	public static final String[] formatMapperLineTerminator = new String[] { "DOS", "UNIX", "CR", "ADQ", "ADQMB",
			"ADQUNIX", "None" };

	/**
	 * Whether to push the output into the output of a servlet with the executeTrans
	 * Carte/DI-Server servlet
	 */
	...
	/**
	 * Setting to allow the enclosure to be always surrounding a String value, even
	 * when there is no separator inside
	 */
	@Injection(name = "FORCE_ENCLOSURE")
	private boolean enclosureForced;

	/**
	 * Setting to allow for backwards compatibility where the enclosure did not show
	 * up at all if Force Enclosure was not checked
	 */
	...
	/**
	 * The file format: DOS or Unix It could be injected using the key "FORMAT" see
	 * the setter {@link TextFileOutputMeta#setFileFormat(java.lang.String)}.
	 */
	private String fileFormat;

	/**
	 * if this value is larger then 0, the text file is split up into parts of this
	 * number of lines
	 */
	...
	/**
	 * @deprecated use {@link #getSplitEvery(VariableSpace)} or
	 *             {@link #getSplitEveryRows()}
	...
	/**
	 * @param encoding The desired encoding of output file, null or empty if the
	...
	/**
	 * @return The desired last line in the output file, null or empty if nothing
	 *         has to be added.
	 */
	...
	/**
	 * @param endedLine The desired last line in the output file, null or empty if
	...
	protected void readData(Node stepnode, IMetaStore metastore) throws KettleXMLException {
		try {
			...
			createparentfolder = (createParentFolderTagValue == null) ? true
			...
			doNotOpenNewFileInit = "Y"
					.equalsIgnoreCase(XMLHandler.getTagValue(stepnode, "file", "do_not_open_new_file_init"));
	...
	public String getNewLine(String fformat) {
		...
		if (fformat != null) {
			if (fformat.equalsIgnoreCase("DOS")) {
				...
				nl = "\r";
			} else if (fformat.equalsIgnoreCase("ADQ")) {
				nl = "\n";
			} else if (fformat.equalsIgnoreCase("ADQMB")) {
				nl = "\u0002\n";
			} else if (fformat.equalsIgnoreCase("ADQUNIX")) {
				nl = "\n";
	...
	public String buildFilename(String filename, String extension, VariableSpace space, int stepnr, String partnr,
		...
		return super.buildFilename(space, realFileName, realExtension, Integer.toString(stepnr), partnr,
				Integer.toString(splitnr), new Date(), ziparchive, true, meta);
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta, StepMeta stepMeta, RowMetaInterface prev,
		...
		if (prev != null && prev.size() > 0) {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
					BaseMessages.getString(PKG, "TextFileOutputMeta.CheckResult.FieldsReceived", "" + prev.size()),
					stepMeta);
			remarks.add(cr);
			...
			if (error_found) {
				error_message = BaseMessages.getString(PKG, "TextFileOutputMeta.CheckResult.FieldsNotFound",
						error_message);
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
				remarks.add(cr);
			} else {
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
						BaseMessages.getString(PKG, "TextFileOutputMeta.CheckResult.AllFieldsFound"), stepMeta);
				remarks.add(cr);
			}
		}
		...
		if (input.length > 0) {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
					BaseMessages.getString(PKG, "TextFileOutputMeta.CheckResult.ExpectedInputOk"), stepMeta);
			remarks.add(cr);
		} else {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG, "TextFileOutputMeta.CheckResult.ExpectedInputError"), stepMeta);
			remarks.add(cr);
		}

		cr = new CheckResult(CheckResultInterface.TYPE_RESULT_COMMENT,
				BaseMessages.getString(PKG, "TextFileOutputMeta.CheckResult.FilesNotChecked"), stepMeta);
		remarks.add(cr);
	}
	...
	/**
	 * Since the exported transformation that runs this will reside in a ZIP file,
	 * we can't reference files relatively. So what this does is turn the name of
	 * the base path into an absolute path.
	 *
	 * @param space                   the variable space to use
	 * @param definitions
	 * @param resourceNamingInterface
	 * @param repository              The repository to optionally load other
	 *                                resources from (to be converted to XML)
	 * @param metaStore               the metaStore in which non-kettle metadata
	 *                                could reside.
	 *
		...
		if (fileName != null && !fileName.isEmpty()) {
			try {
				FileObject fileObject = KettleVFS
						.getFileObject(getParentStepMeta().getParentTransMeta().environmentSubstitute(fileName));
