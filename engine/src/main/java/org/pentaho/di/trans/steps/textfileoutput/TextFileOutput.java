...
public class TextFileOutput extends BaseStep implements StepInterface {
	...
	private static final boolean COMPATIBILITY_APPEND_NO_HEADER = "Y"
			.equals(Const.NVL(System.getProperty(Const.KETTLE_COMPATIBILITY_TEXT_FILE_OUTPUT_APPEND_NO_HEADER), "N"));

	private static final Pattern PATTERN_UNIX = Pattern.compile("[\u0000\u0001\u0007]");
	private static final Pattern PATTERN_ADQ = Pattern.compile("[\u0000\u0001\u0002\u0007]");
	private static final Pattern PATTERN_CR = Pattern.compile("\r");
			...
			try {
				...
				if (writingToFileForFirstTime) { // Opening file for first time
					...
					if ((maxOpenFiles > 0) && (data.getFileStreamsCollection().getNumOpenFiles() >= maxOpenFiles)) {
						...
						// like we're opening the file for the first time, and if we're set up to append
						// to existing files it will cause and
					...
					OutputStream fileOutputStream = getOutputStream(filename, getTransMeta(),
							!isZipFile && appendToExistingFile);
					CompressionOutputStream compressionOutputStream = compressionProvider
							.createOutputStream(fileOutputStream);

					// The compression output stream may also archive entries. For this we create
					// the filename
					// (with appropriate extension) and add it as an entry to the output stream. For
					// providers
					// that do not archive entries, they should use the default no-op
					// implementation.
				...
				} else if (fileStreams.getBufferedOutputStream() == null) { // File was previously opened and now needs
																			// to be reopened.
					int maxOpenFiles = getMaxOpenFiles();
					if ((maxOpenFiles > 0) && (data.getFileStreamsCollection().getNumOpenFiles() >= maxOpenFiles)) {
	...
	// Warning!!!
	// We need to be very particular about how we go about determining whether or
	// not to write a file header before writing the row data.
	// There are two performance issues in play. 1: Don't hit the file system
	// unnecessarily. 2: Don't search the collection of
	// file streams unnecessarily. Messing around with this method could have
	...
	public boolean isWriteHeader(String filename) throws KettleException {
		...
		isWriteHeader &= writingToFileForFirstTime
				&& (!meta.isFileAppended() || (!COMPATIBILITY_APPEND_NO_HEADER && !isFileExists(filename)));
	...
	private boolean writeRowToFile(Object[] row) throws KettleException {
		if (row != null) {
			...
			// If file has reached max user defined size. Close current file and open a new
			// file.
			if (!meta.isFileNameInField() && (getLinesOutput() > 0) && (data.splitEvery > 0)
		...
		try {
			if (meta.getOutputFields() == null || meta.getOutputFields().length == 0) {
				...
				for (int i = 0; i < rowMeta.size(); i++) {
					...
					// no special null value default was specified since no fields are specified at
					// all
	...
	private void writeField(ValueMetaInterface v, Object valueData, byte[] nullString) throws KettleStepException {
		try {
			...
			if (nullString != null && v.isNull(valueData)) {
				...
				if (meta.isFastDump()) {
					if (valueData instanceof byte[] || (v.getOriginalColumnTypeName() != null
							&& v.getOriginalColumnTypeName().contains("BLOB"))) {// binary

						if (valueData instanceof byte[]) {
							str = (byte[]) valueData;
						} else {
							if (meta.getFileFormat().equals("UNIX")) {// MPP platform to binary bytea
								str = getBinaryString("\\x" + Hex.encodeHexString(valueData.toString().getBytes()));
							} else if (meta.getFileFormat().equals("ADQUNIX")) {// Hive platform to BASE64
								str = getBinaryString(
										Base64.getEncoder().encodeToString(valueData.toString().getBytes()));
							} else if (meta.getFileFormat().equals("ADQ")) {// DM, Oracle platform
								str = getBinaryString("0x" + Hex.encodeHexString(valueData.toString().getBytes()));
							} else {
								str = getBinaryString(valueData.toString());
							}
						}
					} else {// string
						if (meta.getFileFormat().equals("UNIX")) {
							str = getBinaryString((valueData == null) ? ""
									: PATTERN_UNIX.matcher(valueData.toString()).replaceAll(""));
						} else if (meta.getFileFormat().matches("(ADQ|ADQUNIX)")) {
							str = getBinaryString((valueData == null) ? ""
									: escapeCRLF(PATTERN_ADQ.matcher(valueData.toString()).replaceAll("")));
						} else {
		...
		try {
			...
			if (meta.getOutputFields() != null && meta.getOutputFields().length > 0) {
				for (int i = 0; i < meta.getOutputFields().length; i++) {
					...
					boolean writeEnclosure = (meta.isEnclosureForced() && data.binaryEnclosure.length > 0 && v != null
							&& v.isString())
				...
				// PDI-17902 - Concat Fields changes the output row meta, only input rows are
				// desired
			...
			if (!meta.isDoNotOpenNewFileInit() && !meta.isFileNameInField()) {
				try {
					...
					if (getParentVariableSpace() == null) {
						logError("Couldn't open file " + KettleVFS.getFriendlyURI(meta.getFileName()) + "."
								+ meta.getExtension(), e);
					} else {
						logError("Couldn't open file "
								+ KettleVFS.getFriendlyURI(
										getParentVariableSpace().environmentSubstitute(meta.getFileName()))
		...
		if (separatorExists || enclosureExists) {
			...
			for (int index = 0; !result && index < source.length; index++) {
				if (enclosureExists && source[index] == enclosure[0]) {
					...
					// Potential match found, make sure there are enough bytes to support a full
					// match
			...
			if (parentfolder.exists()) {
				...
				if (meta.isCreateParentFolder()) {
					...
					throw new KettleException(
							BaseMessages.getString(PKG, "TextFileOutput.Log.ParentFolderNotExistCreateIt",
