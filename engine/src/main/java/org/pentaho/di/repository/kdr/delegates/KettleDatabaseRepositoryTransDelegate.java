...
public class KettleDatabaseRepositoryTransDelegate extends KettleDatabaseRepositoryBaseDelegate {
  ...
  //[2021-12-09 liqiulin] 
  public RowMetaAndData getTransformation( String transName, ObjectId idDirectory ) throws KettleException {
    return repository.connectionDelegate.getOneRow(
      quoteTable( KettleDatabaseRepository.TABLE_R_TRANSFORMATION ),
      quote( KettleDatabaseRepository.FIELD_TRANSFORMATION_NAME ), transName,
      quote( KettleDatabaseRepository.FIELD_TRANSFORMATION_ID_DIRECTORY ), idDirectory );
  }
  ...
  //[2021-12-10 liqiulin]
  public List<TransHopMeta> getTransHopList( ObjectId id_trans, List<StepMeta> steps ) throws KettleException {
	  List<RowMetaAndData> rTransHop = repository.connectionDelegate.getTransHopList(id_trans);
	  List<TransHopMeta> transHopList = new ArrayList<TransHopMeta>();
	  for(RowMetaAndData r : rTransHop) {
		  TransHopMeta rhm = loadTransHopMeta(r, steps);
		  transHopList.add(rhm);
	  }
	  return transHopList;
  }
  ...
  public TransMeta loadTransformation( TransMeta transMeta, String transname, RepositoryDirectoryInterface repdir,
    ...
    synchronized ( repository ) {
      try {
        ...
        RowMetaAndData rTrans = getTransformation( transname,  repdir.getObjectId());
        ...
        if ( rTrans != null && rTrans.getRowMeta() != null && rTrans.getData() != null ) {
        	
        	transMeta.setObjectId(new LongObjectId( rTrans.getInteger( KettleDatabaseRepository.FIELD_TRANSFORMATION_ID_TRANSFORMATION, 0 ) ));
...
//        ObjectId[] stepids = repository.getStepIDs( transMeta.getObjectId() );
          List<StepMeta> stepList = repository.stepDelegate.loadStepMetaList(transMeta.getObjectId(), transMeta.getDatabases(), transMeta
                .getPartitionSchemas());
          
//          ObjectId[] hopids = getTransHopIDs( transMeta.getObjectId() );
          List<TransHopMeta> transHopList = getTransHopList(transMeta.getObjectId(), stepList);
          
          int nrWork = 3 + noteids.length + stepList.size() + transHopList.size();
          ...
          int j = 0;
          for ( StepMeta s : stepList ) {
            if ( log.isDetailed() ) {
              log.logDetailed( BaseMessages.getString( PKG, "TransMeta.Log.LoadingStepWithID" ) + s.getObjectId());
            }
            if ( monitor != null ) {
            ...
            /*
            StepMeta stepMeta =
              repository.stepDelegate.loadStepMeta( s.getObjectId(), transMeta.getDatabases(), transMeta
                .getPartitionSchemas() );
                */
            if ( s.isMissing() ) {
              transMeta.addMissingTrans( (MissingTrans) s.getStepMetaInterface() );
            }
            // In this case, we just add or replace the shared steps.
            // The repository is considered "more central"
            transMeta.addOrReplaceStep( s );
          ...
          loadRepTrans( transMeta, rTrans );// liqiulin
          if ( monitor != null ) {
            monitor.worked( 1 );
          }
          ...
          for ( TransHopMeta hi : transHopList ) {
            if ( hi != null ) {
        ...
        repository.connectionDelegate.setStepAttributesBuffer( null ); // clear the buffer (should be empty anyway) TODO
   ...
   *          // [2021-12-09 liqiuiln]
   */
  private void loadRepTrans( TransMeta transMeta, RowMetaAndData r) throws KettleException {
    try {
  ...
  public TransHopMeta loadTransHopMeta( ObjectId id_trans_hop, List<StepMeta> steps ) throws KettleException {
	  ...
	  return loadTransHopMeta( r, steps );
  }

  public TransHopMeta loadTransHopMeta( RowMetaAndData r, List<StepMeta> steps ) throws KettleException {
    TransHopMeta hopTransMeta = new TransHopMeta();
    try {
      hopTransMeta.setObjectId( new LongObjectId(r.getInteger( "ID_TRANS_HOP", -1L )) );
      ...
      if ( fromStep == null ) {
        log.logError( "Unable to determine source step of transformation hop with ID: " + hopTransMeta.getObjectId() );
