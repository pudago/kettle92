...
public class KettleDatabaseRepositoryDirectoryDelegate extends KettleDatabaseRepositoryBaseDelegate {
  ...
  //[2021-12-22 liqiulin]
  public List<RowMetaAndData> getDirectoryList() throws KettleDatabaseException, KettleValueException {
    List<RowMetaAndData> dirs = new ArrayList<RowMetaAndData>();
    List<Object[]> rows =
      repository.connectionDelegate.getRows( "SELECT * FROM "
      + quoteTable( KettleDatabaseRepository.TABLE_R_DIRECTORY ), 0 );
    for ( Object[] row : rows ) {
      RowMetaAndData rowWithMeta = new RowMetaAndData( repository.connectionDelegate.getReturnRowMeta(), row );
      long id =
        rowWithMeta.getInteger(
          quote( KettleDatabaseRepository.FIELD_DIRECTORY_ID_DIRECTORY ), 0 );
      if ( id > 0 ) {
        dirs.add( rowWithMeta );
      }
    }
    return dirs;
  }
