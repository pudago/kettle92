...
public class KettleDatabaseRepositoryJobEntryDelegate extends KettleDatabaseRepositoryBaseDelegate {
  ...
  private JobEntryCopy loadJobEntryCopy( ObjectId jobId, ObjectId jobEntryCopyId,
  ...
  // [2021-12-13 liqiulin]
  public JobEntryCopy loadJobEntryCopy( RowMetaAndData r,
  List<JobEntryInterface> jobentries, List<DatabaseMeta> databases, List<SlaveServer> slaveServers, String jobname )
      throws KettleException {
JobEntryCopy jobEntryCopy = new JobEntryCopy();
try {
  jobEntryCopy.setObjectId( new LongObjectId( r.getInteger( KettleDatabaseRepository.FIELD_JOBENTRY_COPY_ID_JOBENTRY_COPY, 0 ) ) );
    ...
    jobEntryCopy.setEntry( JobMeta.findJobEntry( jobentries, jobEntryId ) );
    if ( jobEntryCopy.getEntry() == null ) {
        String jet_code = r.getString( KettleDatabaseRepository.FIELD_JOBENTRY_TYPE_CODE, null );
        JobEntryInterface jobEntry = null;
        PluginRegistry registry = PluginRegistry.getInstance();
        PluginInterface jobPlugin = registry.findPluginWithId( JobEntryPluginType.class, jet_code );
        if ( jobPlugin == null ) {
...
//	                loadJobEntryBase( (JobEntryBase) jobEntry, jobEntryId, databases, slaveServers );
            //[2021-12-20 liqiulin]
            loadJobEntryBase( (JobEntryBase) jobEntry, jobEntryId, 
                r.getString( "NAME", null ), r.getString( "DESCRIPTION", null ), r.getString( "CODE", null ) );
            ( (JobEntryBase) jobEntry ).setAttributesMap( loadJobEntryAttributesMap( 
                new LongObjectId( r.getInteger( KettleDatabaseRepository.FIELD_JOBENTRY_ID_JOB, 0 ) ), jobEntryId ) );
          }
          compatibleJobEntryLoadRep( jobEntry, repository, jobEntryTypeId, databases, slaveServers );
          jobEntry.loadRep( repository, repository.metaStore, jobEntryId, databases, slaveServers );
          jobEntryCopy.getEntry().setObjectId( jobEntryId );
  ...
  throw new KettleException( "Unable to load job entry copy from repository with id_jobentry_copy="
    + (new LongObjectId( r.getInteger( KettleDatabaseRepository.FIELD_JOBENTRY_COPY_ID_JOBENTRY_COPY, 0 ))), dbe );
}
}

public List<JobEntryCopy> loadJobEntryCopyList(ObjectId jobId,
List<JobEntryInterface> jobentries, List<DatabaseMeta> databases,
List<SlaveServer> slaveServers, String jobname) throws KettleException {

List<JobEntryCopy> stepMetaList = new ArrayList<JobEntryCopy>();
List<RowMetaAndData> rJec = repository.connectionDelegate.getJobentryCopies(jobId);
for (RowMetaAndData r : rJec) {
JobEntryCopy jec;
jec = loadJobEntryCopy(r, jobentries, databases, slaveServers, jobname);
stepMetaList.add(jec);
...
return stepMetaList;
  ...
  // [2021-12-20 liqiulin]
	public void loadJobEntryBase(JobEntryBase jobEntryBase, ObjectId jobentryId, String name,
  String description, String jobentryTypeCode) throws KettleException {
    jobEntryBase.setName(name);
    jobEntryBase.setDescription(description);
    jobEntryBase.setPluginId(jobentryTypeCode);
}
