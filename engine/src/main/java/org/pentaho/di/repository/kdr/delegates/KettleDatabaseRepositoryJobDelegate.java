...
public class KettleDatabaseRepositoryJobDelegate extends KettleDatabaseRepositoryBaseDelegate {
  ...
  public JobMeta loadJobMeta( String jobname, RepositoryDirectoryInterface repdir, ProgressMonitorListener monitor ) throws KettleException {
    ...
    synchronized ( repository ) {
      try {
        ...
        if ( jobMeta.getObjectId() != null ) {
          repository.connectionDelegate.fillJobentryAttributesBuffer( jobMeta.getObjectId() ); // read all the attributes
          ...
          List<JobEntryCopy> jecList = repository.jobEntryDelegate.loadJobEntryCopyList(jobMeta.getObjectId(), 
        		  jobentries, jobMeta.getDatabases(), jobMeta.getSlaveServers(), jobname);
          
          // ObjectId[] hopid = repository.getJobHopIDs( jobMeta.getObjectId() );
          List<JobHopMeta> jobHopList = loadJobHopMetaList(jobMeta.getObjectId(), jecList);

          int nrWork = 2 + noteids.length + jecList.size() + jobHopList.size();
          if ( monitor != null ) {
          ...
          if ( log.isDetailed() ) {
            log.logDetailed( "Loading " + jecList.size() + " job entries" );
          }
          for ( int i = 0; i < jecList.size(); i++ ) {
            if ( monitor != null ) {
              monitor.subTask( BaseMessages.getString( PKG, "JobMeta.Monitor.ReadingJobEntryNr" )
                + ( i + 1 ) + "/" + ( jecList.size() ) );
            }

            JobEntryCopy jec = jecList.get(i);
          ...
          // Load the hops...
          if ( log.isDetailed() ) {
            log.logDetailed( "Loading " + jobHopList.size() + " job hops" );
          }
          for ( int i = 0; i < jobHopList.size(); i++ ) {
            if ( monitor != null ) {
              monitor.subTask( BaseMessages.getString( PKG, "JobMeta.Monitor.ReadingJobHopNr" )
                + ( i + 1 ) + "/" + ( jecList.size() ) );
            }
            JobHopMeta hi = jobHopList.get(i);
          ...
          repository.connectionDelegate.setJobentryAttributesBuffer( null ); // clear the buffer (should be empty anyway) 
  ...
  public JobHopMeta loadJobHopMeta( RowMetaAndData r, List<JobEntryCopy> jobcopies ) throws KettleException {
    JobHopMeta jobHopMeta = new JobHopMeta();
    try {
      if ( r != null ) {
        long id_jobentry_copy_from = r.getInteger( "ID_JOBENTRY_COPY_FROM", -1L );
        ...
        throw new KettleException( "Unable to find job hop with ID : " + r.getString("ID_JOB_HOP", null) );
      }
    } catch ( KettleDatabaseException dbe ) {
      throw new KettleException( BaseMessages.getString( PKG, "JobHopMeta.Exception.UnableToLoadHopInfoRep", ""
        + r.getString("ID_JOB_HOP", null) ), dbe );

    }
  }

public List<JobHopMeta> loadJobHopMetaList( ObjectId id_job, List<JobEntryCopy> jobcopies ) throws KettleException {
  List<RowMetaAndData> rJobHop = repository.connectionDelegate.getJobHopList(id_job);
  List<JobHopMeta> jobHopList = new ArrayList<JobHopMeta>();
  for(RowMetaAndData r : rJobHop) {
    JobHopMeta rhm = loadJobHopMeta(r, jobcopies);
    jobHopList.add(rhm);
  }
  return jobHopList;
}
