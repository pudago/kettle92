...
public class KettleDatabaseRepositoryStepDelegate extends KettleDatabaseRepositoryBaseDelegate {
  ...
  public StepMeta loadStepMeta( ObjectId stepId, List<DatabaseMeta> databases,
	  ...
	  return loadStepMeta( r, databases,partitionSchemas );
  }

  //[2021-12-09 liqiulin]
  public List<StepMeta> loadStepMetaList( ObjectId id_trans, List<DatabaseMeta> databases,
		    List<PartitionSchema> partitionSchemas ) throws KettleException {
	  List<StepMeta> stepMetaList = new ArrayList<StepMeta>();
	  List<RowMetaAndData> rSteps = repository.connectionDelegate.getSteps(id_trans);
	  for(RowMetaAndData r: rSteps) {
		  StepMeta sm = loadStepMeta( r, databases, partitionSchemas );
		  stepMetaList.add(sm);
	  }
	  return stepMetaList;
  }
   ...
   */
  public StepMeta loadStepMeta( RowMetaAndData r, List<DatabaseMeta> databases,
    List<PartitionSchema> partitionSchemas ) throws KettleException {
    ...
    try {
      //
      if ( r != null ) {
        stepMeta.setObjectId( new LongObjectId(r.getInteger( KettleDatabaseRepository.FIELD_STEP_ID_STEP, -1L )) );
...
//        stepMeta.setStepID( steptyperow.getString( KettleDatabaseRepository.FIELD_STEP_TYPE_CODE, null ) );
        stepMeta.setStepID( r.getString( KettleDatabaseRepository.FIELD_STEP_TYPE_CODE, null ) );
        ...
        //
        String rowDistributionCode = repository.getStepAttributeString( stepMeta.getObjectId(), 0, "row_distribution_code" );
        ...
        //
        stepMeta.setAttributesMap( loadStepAttributesMap( stepMeta.getObjectId() ) );
        ...
        throw new KettleException( BaseMessages.getString(
          PKG, "StepMeta.Exception.StepInfoCouldNotBeFound", String.valueOf( stepMeta.getObjectId() ) ) );
      }
