...
public class KettleDatabaseRepositoryDatabaseDelegate extends KettleDatabaseRepositoryBaseDelegate {
  ...
  // [2021-12-21 liqiulin]
  public Collection<RowMetaAndData> getDatabaseList() throws KettleDatabaseException, KettleValueException {
    ...
    List<Object[]> rows =
      repository.connectionDelegate.getRows( "SELECT A.*, B.CODE AS TYPE_CODE, C.CODE AS CONTYPE_CODE FROM "
        ...
        + quoteTable( KettleDatabaseRepository.TABLE_R_DATABASE_CONTYPE )
        + " C ON A.ID_DATABASE_CONTYPE = C.ID_DATABASE_CONTYPE", 0 );
    for ( Object[] row : rows ) {
      RowMetaAndData rowWithMeta = new RowMetaAndData( repository.connectionDelegate.getReturnRowMeta(), row );
      long id =
        rowWithMeta.getInteger(
          quote( KettleDatabaseRepository.FIELD_DATABASE_ID_DATABASE ), 0 );
      if ( id > 0 ) {
        dbs.add( rowWithMeta );
      }
    }
  ...
  private Map<String, String> rows2Props(List<RowMetaAndData> rows) throws KettleValueException{
	  ...
	  for (RowMetaAndData r : rows) {
		  props.put(r.getString( KettleDatabaseRepository.FIELD_DATABASE_ATTRIBUTE_CODE, "" ), 
				  r.getString( KettleDatabaseRepository.FIELD_DATABASE_ATTRIBUTE_VALUE_STR, "" ));
	  }
	  return props;
  }
  ...
  public List<DatabaseMeta> loadDatabaseMetaList( ) throws KettleException {
	  ...
	  Collection<RowMetaAndData> rDbs = getDatabaseList();
	  ...
	  Collection<RowMetaAndData> rDbAttrs = getDatabaseAttributes();
