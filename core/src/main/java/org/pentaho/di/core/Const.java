...
public class Const {
  ...
  public static String getIPAddress() throws Exception {
    ...
    */
    String ip = "127.0.0.1";
		if (!InetAddress.getLocalHost().getHostName().matches("^localhost(\\..*)?")) {
			ip = InetAddress.getLocalHost().getHostAddress();//real IP
		} else {
			try (final DatagramSocket socket = new DatagramSocket()) {
				socket.connect(InetAddress.getByName("8.8.8.8"), 10000);
				ip = socket.getLocalAddress().getHostAddress();//out IP
			}
		}
		return ip;
  }
